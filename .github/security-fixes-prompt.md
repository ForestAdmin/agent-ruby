...
For **npm-ecosystem** alerts (the tooling `package.json`/`yarn.lock` at the root): fix with yarn (`yarn why <pkg>`, bump or root-`resolutions` pin bounded to the parent-compatible major, `yarn install` to update the committed `yarn.lock`). For **github-actions-ecosystem** alerts: bump the `uses:` reference in the affected `.github/workflows/*.yml`; `SECURITY_GH_PAT` carries the `workflows:write` permission this push requires — if the push is nevertheless rejected citing workflow permissions, unstage those changes, move the alerts to "Could not auto-fix (token lacks Workflows permission)", and ship the rest.
...
Deferred / Security pins added / Security pins removed / Could not auto-fix are informational — no checkbox column.
