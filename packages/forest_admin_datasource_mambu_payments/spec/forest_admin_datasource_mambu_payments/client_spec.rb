    ...
    it 'surfaces the HTTP status and the Numeral error message on a 422' do
      stub_request(:post, "#{base}/payment_orders")
        .to_return(status: 422,
                   body: { 'error' => { 'message' => 'iban is invalid' } }.to_json,
                   headers: { 'Content-Type' => 'application/json' })

      expect { client.create_payment_order({}) }
        .to raise_error(ForestAdminDatasourceMambuPayments::APIError, /HTTP 422.*iban is invalid/)
    end

    it 'exposes the status and parsed body on the error object' do
      stub_request(:post, "#{base}/payment_orders")
        .to_return(status: 422, body: { 'message' => 'nope' }.to_json,
                   headers: { 'Content-Type' => 'application/json' })

      client.create_payment_order({})
    rescue ForestAdminDatasourceMambuPayments::APIError => e
      expect(e.status).to eq(422)
      expect(e.body).to eq('message' => 'nope')
    end
  end
end
