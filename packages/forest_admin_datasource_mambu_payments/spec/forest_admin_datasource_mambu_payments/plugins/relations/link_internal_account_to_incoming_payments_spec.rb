RSpec.describe ForestAdminDatasourceMambuPayments::Plugins::Relations::LinkInternalAccountToIncomingPayments do
  ...
  describe '#run' do
    it 'adds OneToMany incoming_payments on MambuInternalAccount via internal_account_id' do
      install
      rel = datasource_customizer.collections['MambuInternalAccount'].one_to_many_relations['incoming_payments']
    ...
    it 'does not customize MambuIncomingPayment (FK + ManyToOne already native)' do
      install
      expect(datasource_customizer.collections.keys).to contain_exactly('MambuInternalAccount')
    end
