RSpec.describe ForestAdminDatasourceMambuPayments::Plugins::Relations::LinkPaymentOrderToReturns do
  ...
  describe '#run' do
    it 'adds OneToMany returns on MambuPaymentOrder via related_payment_id' do
      install
      rel = datasource_customizer.collections['MambuPaymentOrder'].one_to_many_relations['returns']
    ...
    it 'does not customize MambuReturn (FK + API filter already native)' do
      install
      expect(datasource_customizer.collections.keys).to contain_exactly('MambuPaymentOrder')
    end
