...
# Filter / Page / Sort / Aggregation / Leaf / Caller as bare names without
# leaking constants into the top-level namespace. Same pattern as the
# active_record specs.
  ...
  RSpec.describe Collections::Ticket do
    ...
    describe 'schema' do
      ...
      it 'declares ManyToOne relations to User and Organization' do
        expect(collection.schema[:fields]['requester'])
          .to be_a(ForestAdminDatasourceToolkit::Schema::Relations::ManyToOneSchema)
        expect(collection.schema[:fields]['organization'])
          .to be_a(ForestAdminDatasourceToolkit::Schema::Relations::ManyToOneSchema)
      end
    ...
    describe '#list' do
      ...
      context 'when filtering by id (PK lookup)' do
        it 'short-circuits to a single bulk fetch' do
          ...
          allow(client).to receive(:fetch_tickets_by_ids).with([215]).and_return(215 => ticket)
        ...
        it 'falls back to search when the id leaf uses an unsupported operator' do
          ...
          allow(client).to receive(:fetch_tickets_by_ids)
          ...
          expect(client).not_to have_received(:fetch_tickets_by_ids)
        end
      end

      context 'when filtering by other fields' do
        it 'translates the condition tree to a Zendesk Search query' do
          allow(client).to receive_messages(search: tickets, fetch_user_emails: { 10 => 'a@b.com' })

          filter = Filter.new(condition_tree: Leaf.new('status', 'equal', 'open'))
          collection.list(nil, filter, ['id', 'subject'])

          expect(client).to have_received(:search).with('ticket', hash_including(query: 'status:open'))
        end
        ...
        it 'merges filter.search into the query string' do
          ...
          expect(client).to have_received(:search)
            .with('ticket', hash_including(query: 'status:open password reset'))
        end
        ...
        it 'forwards sort_by and sort_order' do
          ...
          expect(client).to have_received(:search)
            .with('ticket', hash_including(sort_by: 'created_at', sort_order: 'asc'))
        end
        ...
        it 'sends sort_order=desc when sort.ascending is false' do
          ...
          expect(client).to have_received(:search).with('ticket', hash_including(sort_order: 'desc'))
        end
        ...
        it 'returns no sort_by when the field is not in Zendesk allow-list' do
          ...
          expect(client).to have_received(:search)
            .with('ticket', hash_including(sort_by: nil, sort_order: nil))
        end
        ...
        it 'translates page offset/limit to Zendesk page/per_page' do
          ...
          expect(client).to have_received(:search).with('ticket', hash_including(page: 3, per_page: 15))
        end
      ...
      describe 'requester_email enrichment' do
        ...
        it 'bulk-fetches emails when requester_email is in the projection' do
          allow(client).to receive(:fetch_user_emails)
          ...
          expect(client).to have_received(:fetch_user_emails).with([10, 10, 20])
        end

        it 'skips bulk fetch when requester_email is not requested' do
          allow(client).to receive(:fetch_user_emails)
          collection.list(nil, Filter.new, ['id', 'subject'])
          expect(client).not_to have_received(:fetch_user_emails)
        end

        it 'fetches by default when projection is nil' do
          allow(client).to receive_messages(fetch_user_emails: {}, fetch_ticket_comments: [],
                                            fetch_users_by_ids: {})
          collection.list(nil, Filter.new, nil)
          expect(client).to have_received(:fetch_user_emails)
        end
      end
      ...
      describe 'relation embedding (ManyToOne)' do
        ...
        it 'embeds requester when requester:* is in projection' do
          allow(client).to receive(:fetch_user_emails).and_return({})
          allow(client).to receive(:fetch_users_by_ids)
            .with([10, 11])
          ...
          expect(client).to have_received(:fetch_users_by_ids).with([10, 11])
        end

        it 'embeds organization when organization:* is in projection' do
          allow(client).to receive(:fetch_user_emails).and_return({})
          allow(client).to receive(:fetch_organizations_by_ids)
            .with([50])
          ...
          expect(client).to have_received(:fetch_organizations_by_ids).with([50])
        end
        ...
        it 'skips relation fetches when only column projection is requested' do
          ...
          expect(client).not_to have_received(:fetch_users_by_ids)
          expect(client).not_to have_received(:fetch_organizations_by_ids)
        end
      ...
      describe 'comments embedding' do
        ...
        it 'does not fetch comments when projection excludes them' do
          allow(client).to receive(:fetch_ticket_comments)
          collection.list(nil, Filter.new, ['id', 'subject'])
          expect(client).not_to have_received(:fetch_ticket_comments)
        end

        it 'fetches comments when projection requests the field' do
          allow(client).to receive(:fetch_ticket_comments).with(7).and_return([
                                                                                { 'id' => 11, 'body' => 'hi',
                                                                                  'html_body' => '<p>hi</p>',
        ...
        it 'fetches comments when projection requests a sub-field of comments' do
          ...
          expect(client).to have_received(:fetch_ticket_comments).with(7)
        end

        it 'leaves author_email/author_name nil when the author cannot be resolved' do
          allow(client).to receive_messages(fetch_ticket_comments: [{ 'id' => 1, 'author_id' => 999 }],
                                            fetch_users_by_ids: {})
    ...
    describe 'timezone plumbing' do
      ...
      it 'feeds caller.timezone into the translator (Date filter shifts by TZ)' do
        ...
        allow(client).to receive_messages(search: [], fetch_user_emails: {})

        filter = Filter.new(condition_tree: Leaf.new('created_at', 'after', Date.new(2026, 1, 15)))
        collection.list(paris_caller, filter, ['id'])

        expect(client).to have_received(:search)
          .with('ticket', hash_including(query: 'created_at>2026-01-14T23:00:00Z'))
      end
      ...
      it 'falls back to UTC when caller is nil' do
        ...
        expect(client).to have_received(:search)
          .with('ticket', hash_including(query: 'created_at>2026-04-27T00:00:00Z'))
      end
    ...
    describe '#aggregate' do
      it 'returns the count under string keys' do
        allow(client).to receive(:count).with('ticket', query: 'status:open').and_return(7)
    ...
    describe '#update' do
      it 'updates every id resolved from the filter (PK lookup short-circuit)' do
        ...
        expect(client).to have_received(:update_ticket).with(12, hash_including('status' => 'solved'))
      end
      ...
      it 'updates each id when the filter resolves to several' do
        ...
        expect(client).to have_received(:update_ticket).with(1, hash_including('priority' => 'high'))
        expect(client).to have_received(:update_ticket).with(2, hash_including('priority' => 'high'))
      end
      ...
      it 'silently drops description on update (no comment write path)' do
        ...
        allow(client).to receive(:update_ticket) do |id, payload|
          expect(id).to eq(7)
          expect(payload).not_to have_key('comment')
          expect(payload).not_to have_key('description')
        end
        ...
        expect(client).to have_received(:update_ticket)
      end
    end
    ...
    describe '#delete' do
      it 'deletes each id resolved from the filter' do
        ...
        allow(client).to receive(:delete_ticket)

        filter = Filter.new(condition_tree: Leaf.new('id', 'in', [1, 2]))
        collection.delete(nil, filter)

        expect(client).to have_received(:delete_ticket).with(1)
        expect(client).to have_received(:delete_ticket).with(2)
      end
    end
