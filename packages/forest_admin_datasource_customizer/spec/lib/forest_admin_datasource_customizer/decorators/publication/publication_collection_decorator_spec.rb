  ...
  module Decorators
    module Publication
      ...
      describe PublicationCollectionDecorator do
        ...
        context 'when a ManyToMany foreign_key lives on the foreign collection instead of the through collection' do
          # Reproduces #370: a `has_many :through` chaining into a polymorphic
          # `has_one`/`belongs_to` with a custom foreign_key makes the AR datasource emit
          # a ManyToManySchema whose foreign_key column actually lives on the foreign
          # collection ('leaf'), not on the through collection ('child').
          before do
            @collection_parent = instance_double(
              ForestAdminDatasourceToolkit::Collection,
              name: 'parent',
              schema: {
                fields: {
                  'id' => ColumnSchema.new(column_type: 'Number', is_primary_key: true),
                  'leaves' => Relations::ManyToManySchema.new(
                    origin_key: 'owner_id',
                    origin_key_target: 'id',
                    foreign_key: 'custom_child_id',
                    foreign_key_target: 'id',
                    foreign_collection: 'leaf',
                    through_collection: 'child',
            ...
            @collection_child = instance_double(
              ForestAdminDatasourceToolkit::Collection,
              name: 'child',
              schema: {
                fields: {
            ...
            @collection_leaf = instance_double(
              ForestAdminDatasourceToolkit::Collection,
              name: 'leaf',
              schema: {
                fields: {
                  'id' => ColumnSchema.new(column_type: 'Number', is_primary_key: true),
                  'custom_child_id' => ColumnSchema.new(column_type: 'Number')
                }
              }
            )

            through_datasource = ForestAdminDatasourceToolkit::Datasource.new
            through_datasource.add_collection(@collection_parent)
            through_datasource.add_collection(@collection_child)
            through_datasource.add_collection(@collection_leaf)

            @decorated_parent = PublicationDatasourceDecorator.new(through_datasource).get_collection('parent')
          end

          it 'is published without logging a false "field not found" warning' do
            logger = instance_spy(Logger)
            allow(ForestAdminAgent::Facades::Container).to receive(:logger).and_return(logger)

            expect(@decorated_parent.published?('leaves')).to be(true)
            expect(logger).not_to have_received(:log)
          end

          it 'still logs and hides the relation when the foreign key is truly missing everywhere' do
            @collection_leaf.schema[:fields].delete('custom_child_id')
            logger = instance_spy(Logger)
            allow(ForestAdminAgent::Facades::Container).to receive(:logger).and_return(logger)

            result = @decorated_parent.published?('leaves')

            expect(logger).to have_received(:log).with('Warn', "Field 'custom_child_id' not found in schema of collection 'child'")
            expect(result).to be(false)
          end
        end
      end
