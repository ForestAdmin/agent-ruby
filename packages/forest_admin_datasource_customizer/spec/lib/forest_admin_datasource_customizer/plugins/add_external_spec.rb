  ...
  module Plugins
    describe AddExternalRelation do
      ...
      it 'raises an error when options are missing required keys' do
        ...
        end.to raise_error(ForestAdminDatasourceToolkit::Exceptions::BadRequestError, 'The options parameter must contains the following keys: `name, schema, listRecords`')
      end
    end
  end
end
