      ...
      describe FieldsGenerator do
        ...
        describe 'Construction' do
          it 'does not modify the schema' do
            ...
            expect(fields.keys).to eq(%w[_id created_at updated_at title body rating tag_ids user_id user_id__many_to_one co_author])
          end
          ...
          it 'skips flattened models' do
            ...
            expect(fields.keys).to eq(%w[_id name section@@@body])
          end

          it 'flattens all nested fields when no level is provided' do
            stack = [{ prefix: nil, as_fields: %w[section.content section.body], as_models: [] }]
            fields = described_class.build_fields_schema(Label, stack)

            expect(fields.keys).to eq(%w[_id name section@@@content section@@@body])
          end
          ...
          it 'flattens only requested fields' do
            ...
            expect(fields.keys).to eq(%w[_id created_at updated_at label label@@@name])
          end

          it 'onlies flatten selected fields when level = 1' do
            stack = [{ prefix: nil, as_fields: %w[label.name label.section], as_models: [] }]
            fields = described_class.build_fields_schema(Band, stack)

            expect(fields.keys).to eq(%w[_id created_at updated_at label@@@name label@@@section])
          end
