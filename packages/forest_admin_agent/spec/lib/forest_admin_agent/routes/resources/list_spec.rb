    ...
    module Resources
      ...
      describe List do
        ...
        it 'throws an error when the filter operator is not allowed' do
          ...
          expect { list.handle_request(args) }.to raise_error(ForestAdminAgent::Http::Exceptions::UnprocessableError, "The given operator 'shorter_than' is not supported by the column: 'id'. The column is not filterable")
        end
      end
    end
  end
