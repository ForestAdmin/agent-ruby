    ...
    describe QueryValidator do
      ...
      describe 'queries that raise exceptions' do
        it 'raises an error for an empty query' do
          query = '   '
          expect { described_class.valid?(query) }.to raise_error(BadRequestError, 'Query cannot be empty.')
        end

        it 'raises an error for non-SELECT queries' do
          query = 'DELETE FROM users;'
          expect { described_class.valid?(query) }.to raise_error(BadRequestError, 'Only SELECT queries are allowed.')
        end
        ...
        it 'raises an error for unbalanced parentheses outside WHERE clause' do
          ...
          expect { described_class.valid?(query) }.to raise_error(BadRequestError, 'The query contains unbalanced parentheses.')
        end

        it 'raises an error for a semicolon not at the end of the query' do
          query = 'SELECT * FROM users; WHERE id > 1'
          expect { described_class.valid?(query) }.to raise_error(BadRequestError, 'Semicolon must only appear as the last character in the query.')
        end

        it 'raises an error for forbidden keywords even inside subqueries' do
          query = 'SELECT * FROM users WHERE id IN (DROP TABLE users);'
          expect { described_class.valid?(query) }.to raise_error(BadRequestError, 'The query contains forbidden keyword: DROP.')
        end
        ...
        it 'raises an error for an OR-based injection' do
          ...
          expect { described_class.valid?(query) }
            .to raise_error(BadRequestError, 'The query contains a potential SQL injection pattern.')
        end
      end
    end
  end
