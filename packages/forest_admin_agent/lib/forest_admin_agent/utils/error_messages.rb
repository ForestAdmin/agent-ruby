module ForestAdminAgent
  module Utils
    class ErrorMessages
      AUTH_SECRET_MISSING = 'Your Forest authSecret seems to be missing. Can you check that you properly set a Forest authSecret in the Forest initializer?'.freeze

      SECRET_AND_RENDERINGID_INCONSISTENT = 'Cannot retrieve the project you\'re trying to unlock. The envSecret and renderingId seems to be missing or inconsistent.'.freeze

      SERVER_DOWN = 'Cannot retrieve the data from the Forest server. Forest API seems to be down right now.'.freeze

      SECRET_NOT_FOUND = 'Cannot retrieve the data from the Forest server. Can you check that you properly copied the Forest envSecret in the Liana initializer?'.freeze
      ...
      INVALID_STATE_FORMAT = 'Invalid response from the authentication server: the state parameter is not at the right format'.freeze

      INVALID_STATE_RENDERING_ID = 'Invalid response from the authentication server: the state does not contain a renderingId'.freeze
