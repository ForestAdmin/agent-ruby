  ...
  module Utils
    module QueryValidator
      ...
      def self.valid?(query)
        query = query.strip
        raise ForestAdminDatasourceToolkit::Exceptions::BadRequestError, 'Query cannot be empty.' if query.empty?
      ...
      class << self
        ...
        def check_select_only(query)
          ...
          raise ForestAdminDatasourceToolkit::Exceptions::BadRequestError, 'Only SELECT queries are allowed.'
        end
        ...
        def check_semicolon_placement(query)
          ...
          raise ForestAdminDatasourceToolkit::Exceptions::BadRequestError,
                'Semicolon must only appear as the last character in the query.'
        end
        ...
        def check_sql_injection_patterns(query)
          INJECTION_PATTERNS.each do |pattern|
            ...
            raise ForestAdminDatasourceToolkit::Exceptions::BadRequestError,
                  'The query contains a potential SQL injection pattern.'
          end
        end
