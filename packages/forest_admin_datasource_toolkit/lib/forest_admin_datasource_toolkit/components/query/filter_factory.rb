      ...
      class FilterFactory
        ...
        def self.get_previous_period_filter(filter, timezone)
          ...
          def self.make_through_filter(collection, id, relation_name, caller, base_foreign_filter)
            ...
            origin_value = ForestAdminDatasourceToolkit::Utils::Collection.get_through_target(collection, relation_name)
            ...
            Filter.new(
              condition_tree: ConditionTreeFactory.intersect([
                                                               ConditionTreeLeaf.new(relation.origin_key, Operators::EQUAL, origin_value),
                                                               # only the children which match the conditions in base_foreign_filter
                                                               ConditionTreeLeaf.new(relation.foreign_key, Operators::IN, records.map { |record| record[relation.foreign_key_target] })
                                                             ])
            )
          end
